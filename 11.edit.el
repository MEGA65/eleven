...
'--------------------------
  ...
  gosub read_defaults_from_mailbox
  background bg
  border bo
  foreground fg
...
.main_loop
    ...
    bend:else begin  ' else not ctrl_or_alt
      ...
      bend:else begin
        if esc_flag%=1 then begin  ' -- check escape characters
...
' --- merge lines
  ...
  cursor 0,y_pos-y_offs+1:print chr$(27)+"d";
...
' --- help
  ...
  get key t$:gosub redraw_screen:gosub build_status_bar:return


.read_defaults_from_mailbox
'--------------------------
...
.more_help
'---------
  if rwindow(1)=25 then print "{rvon}-- press any key for next page{rvof}";:get keyt$:print "{home}{home}{clr}";:gosub clear_status_bar:else print "{down}{down}{down}";
  ...
  print "  - Then use {rvon}esc,p{rvof} to paste clipboard contents"
  return
